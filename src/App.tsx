...
const App: React.FC<{
  state: {
    bubblePosition: { bubbleX: number; bubbleY: number };
    moveBubble: (x: any, y: any) => void;
  };
}> = (props) => {
  var bubblePosition: { bubbleX: number; bubbleY: number } =
    props.state.bubblePosition;

  let onClickHandler = (x: number, y: number) => {
    props.state.moveBubble(x, y);
  };

  function renderEmptyBubble(i: number, { bubbleX, bubbleY }: type) {
    const x = i % 6;
    const y = Math.floor(i / 6);
    const isWordHere = x === bubbleX && y === bubbleY;
    let piece: any;
    if (isWordHere) {
      piece = <WordBubble key={i} onClick={onClickHandler} />;
    } else {
      piece = (
        <div
          key={i}
  ...
  function emptyBubbles() {
    const bubbles = [];
    for (let i = 0; i < 12; i++) {
      bubbles.push(renderEmptyBubble(i, bubblePosition));
    }
    return <div className="wordConteiner">{bubbles}</div>;
  }
  ...
  return (
    <div className="App">
      ...
      <div className="dndCont">
        <div className="wordConteiner"></div>
        <div>{emptyBubbles()}</div>
      </div>
...
/*{wordConteiners.map(wordConteiner =>
 ...
 const [wordConteiners, setWordConteiners] = useState([
  ...
  function dropWordConteinerHandler(
    e: React.DragEvent<HTMLDivElement>,
    wordConteiner: { id: number; words: { id: number; word: string }[] }
  ): void {
